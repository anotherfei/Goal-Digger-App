...
;------------------------------------------------------------
;	lcdlib.c:14: unsigned char LCD_ready(void) {
;	-----------------------------------------
;	 function LCD_ready
;	-----------------------------------------
_LCD_ready:
...
;------------------------------------------------------------
;	lcdlib.c:17: void LCD_Init(void) {
;	-----------------------------------------
;	 function LCD_Init
;	-----------------------------------------
_LCD_Init:
;	lcdlib.c:18: LCD_functionSet();
	lcall	_LCD_functionSet
;	lcdlib.c:19: LCD_entryModeSet(1, 0); /* increment and no shift */
	mov	dpl, #0x06
	lcall	_LCD_IRWrite
;	lcdlib.c:20: LCD_displayOnOffControl(1, 1, 1); /* display on, cursor on and blinking on */
	mov	dpl, #0x0f
	lcall	_LCD_IRWrite
;	lcdlib.c:21: lcd_ready = 1;
	mov	_lcd_ready,#0x01
;	lcdlib.c:22: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	lcdlib.c:23: void LCD_IRWrite(char c) {
;	-----------------------------------------
;	 function LCD_IRWrite
;	-----------------------------------------
_LCD_IRWrite:
	mov	r7, dpl
;	lcdlib.c:24: lcd_ready = 0;
	mov	_lcd_ready,#0x00
;	lcdlib.c:25: DB = (c & 0xf0); // high nibble, keep RS low
	mov	a,#0xf0
	anl	a,r7
	mov	_P1,a
;	lcdlib.c:26: E = 1;  // pulse E
;	assignBit
	setb	_P1_2
;	lcdlib.c:27: E = 0;
;	assignBit
	clr	_P1_2
;	lcdlib.c:28: DB = (c << 4); // low nibble, keep RS low
	mov	a,r7
	swap	a
	anl	a,#0xf0
	mov	_P1,a
...
;	assignBit
	clr	_P1_2
;	lcdlib.c:31: delay(DELAY_AMOUNT);
	mov	dpl, #0x28
	push	ar7
	lcall	_delay
	pop	ar7
;	lcdlib.c:32: if ((c == CLEAR_DISPLAY) || (c == RETURN_HOME)) {
	cjne	r7,#0x01,00112$
	sjmp	00101$
00112$:
	cjne	r7,#0x02,00102$
00101$:
;	lcdlib.c:33: delay(255);
	mov	dpl, #0xff
	lcall	_delay
;	lcdlib.c:34: delay(255);
	mov	dpl, #0xff
	lcall	_delay
;	lcdlib.c:35: delay(255);
	mov	dpl, #0xff
	lcall	_delay
00102$:
;	lcdlib.c:37: lcd_ready = 1;
	mov	_lcd_ready,#0x01
;	lcdlib.c:38: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	lcdlib.c:70: void LCD_write_string(char* str) {
;	-----------------------------------------
...
00101$:
	...
	mov	r4,a
	jz	00104$
;	lcdlib.c:72: LCD_write_char(*str);
	mov	dpl, r4
	push	ar7
	push	ar6
	push	ar5
