...
class _CommunityPageState extends State<CommunityPage> {
  ...
  Widget _buildFriendsTab(BuildContext context) {
    final leaderboard = <MapEntry<String, int>>[
      ...
      for (var i = 0; i < widget.friends.length; i++)
        MapEntry(widget.friends[i], max(2, widget.streak - i + 2)),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            for (var i = 0; i < leaderboard.length; i++)
              ListTile(
                leading: CircleAvatar(
                    backgroundColor: i == 0 ? gdAccentSoft : gdPrimarySoft,
                    child: Text('#${i + 1}',
                        style: const TextStyle(fontWeight: FontWeight.w900))),
                title: Text(leaderboard[i].key,
                    style: const TextStyle(fontWeight: FontWeight.w900)),
                trailing:
                    Chip(label: Text('${leaderboard[i].value} day streak')),
              ),
          ]),
        ),
      ),
      ...
      SectionTitle(
          title: 'My friend list', trailing: '${widget.friends.length}'),
      const SizedBox(height: 10),
        ...
        AppCard(
          ...
          child: ListTile(
            leading: const CircleAvatar(
                ...
                child: Icon(Icons.person_rounded, color: gdPrimary)),
            title: Text(friend,
                style: const TextStyle(fontWeight: FontWeight.w900)),
            subtitle: const Text('Accountability friend · progress visible',
                style: TextStyle(color: gdMuted, fontWeight: FontWeight.w700)),
            trailing: Wrap(spacing: 4, children: [
              IconButton.filledTonal(
                  tooltip: 'Chat',
                  onPressed: () =>
                      _showSimpleChat(context, friend, isCommunity: false),
                  icon: const Icon(Icons.chat_bubble_rounded)),
              IconButton(
                  tooltip: 'Delete friend',
                  onPressed: () => widget.onDeleteFriend(friend),
                  icon:
                      const Icon(Icons.delete_outline_rounded, color: gdError)),
            ]),
          ),
        ),
      const SizedBox(height: 18),
      ...
      TextField(
          ...
          decoration: const InputDecoration(
              labelText: 'Search friend username', hintText: 'Example: @maya'),
          onSubmitted: (value) {
            if (value.trim().isNotEmpty) widget.onAddFriend(value.trim());
          }),
      const SizedBox(height: 12),
      SectionTitle(title: 'Friend suggestions'),
      const SizedBox(height: 10),
      for (final name in widget.friendSuggestions
          .where((name) => !widget.friends.contains(name)))
        FriendSuggestionCard(
            name: name,
            match: 90 - widget.friendSuggestions.indexOf(name) * 6,
            onAdd: () => widget.onAddFriend(name)),
    ]);
  }
  ...
  Widget _buildCommunitiesTab(BuildContext context) {
    ...
    final leaderboard = [...widget.communities]
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      AppCard(
        child: Padding(
          ...
          child:
            ...
            const Text('Create or join community',
                style: TextStyle(
            ...
            for (var i = 0; i < min(3, leaderboard.length); i++)
              ListTile(
                ...
                title: Text(leaderboard[i].name,
                    style: const TextStyle(fontWeight: FontWeight.w900)),
                subtitle: Text('${leaderboard[i].members} members',
                    style: const TextStyle(
                        color: gdMuted, fontWeight: FontWeight.w700)),
                trailing:
                    Chip(label: Text('${leaderboard[i].similarity}% fit')),
              ),
          ]),
        ),
      ),
      ...
      if (joined.isEmpty)
        const HelpfulErrorBox(
            ...
            message:
                'Join a suggested community below or create your own group.',
            actionLabel: 'Got it',
            showAction: false)
      else
            ...
            child: ListTile(
              ...
              trailing: Wrap(spacing: 4, children: [
                IconButton.filledTonal(
                    tooltip: 'Chat',
                    onPressed: () =>
                        _showSimpleChat(context, group.name, isCommunity: true),
                    icon: const Icon(Icons.chat_bubble_rounded)),
                IconButton(
      ...
      const SizedBox(height: 10),
      ...suggested.map((group) => CommunityMatchCard(
          group: group, onJoin: () => widget.onJoinCommunity(group))),
    ]);
  }

  void _showSimpleChat(BuildContext context, String title,
      ...
      builder: (context) => SafeArea(
        child: Padding(
          ...
          child: Column(
              ...
              children: [
                Text(isCommunity ? '$title chat' : 'Chat with $title',
                ...
                AppCard(
                    ...
                    child: const Padding(
                        ...
                        child: Text(
                            'Say hi, share your goal update, or ask for accountability.',
                            style: TextStyle(
                                color: gdMuted, fontWeight: FontWeight.w700)))),
                const SizedBox(height: 12),
                TextField(
  ...
  Widget build(BuildContext context) {
    return AppCard(
      ...
      child: ListTile(
        ...
        subtitle: Text('$match% similar goals · active this week',
            style:
...
class CommunityMatchCard extends StatelessWidget {
  const CommunityMatchCard(
      {super.key, required this.group, required this.onJoin});
    ...
    return AppCard(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('${group.members} members · ${group.tag}',
              style:
